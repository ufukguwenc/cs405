...
class MeshDrawer {
	...
	constructor() {
		...
		// lightning variables initialization
		this.enableLighting(false); // lighting is initially disabled
		this.ambientLight = 0.5; // intiial light density (since light density slider is starts from 0.5)
...
const meshFS = `
			...
			void main()
			{
				...
				vec4 diffuseC = vec4(diff, diff, diff, 1.0) * texColor;

				if (showTex && enableLighting) {
					gl_FragColor = ambientColor + diffuseC;
				} else if (showTex) {
					// without lighting texture
